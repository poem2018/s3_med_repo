...
class BM25Retriever(BaseRetriever):
    ...
    def _search(self, query: str, num: int = None, return_score: bool = False):
        ...
        if self.contain_doc:
            all_contents = [
                ##content revise
                f"{json.loads(self.searcher.doc(hit.docid).raw())['title']} {json.loads(self.searcher.doc(hit.docid).raw())['text']}" 
                for hit in hits
            ]
            results = [
                {
...
@app.get("/")
def read_root():
    return {"message": "Retrieval Server is running", "usage": "Send POST request to /retrieve"}

@app.get("/test")
def test_retrieve():
    # 简单的测试查询
    test_queries = ["machine learning", "artificial intelligence"]
    results, scores = retriever.batch_search(
    ...
    return {"test_queries": test_queries, "result": resp}
