...
Find similar patient cases and use them to predict ICU mortality without RAG
"""
...
def create_similar_cases_prompt(patient_text, similar_cases):
    """Create a prompt using only similar cases (no RAG)"""
    prompt = f"""You are an expert ICU physician. Analyze the patient's data using similar historical cases to predict ICU mortality.
...
- Look for patterns between the current patient and similar historical cases
...
=== TASK ===
Based on the similar historical cases above:

1. Compare the current patient with similar historical cases
2. Identify patterns and risk factors from the similar cases
...
EVIDENCE: [Cite supporting evidence from similar cases]
...
def main():
    parser = argparse.ArgumentParser(description="Baseline 5: Similar Cases Only for ICU Mortality")
    parser.add_argument("--data_dir", default="/scratch/bcew/ruikez2/intern/s3_med/data/baseline_text",
                        help="Directory containing patient data")
    parser.add_argument("--output_dir", default="/scratch/bcew/ruikez2/intern/s3_med/results/baseline_5",
    ...
    print(f"Processing {len(patients_text)} patients with Similar Cases Only...")
    ...
    for i, patient in enumerate(tqdm(patients_text, desc="Processing patients")):
        ...
        # Create prompt with only similar cases (no RAG)
        prompt = create_similar_cases_prompt(patient_text, similar_cases_text)
    ...
    output_file = os.path.join(args.output_dir, 'baseline_5_similar_cases_results.json')
    ...
    # Print summary
    print("\n" + "="*50)
    print("BASELINE 5: SIMILAR CASES ONLY")
    print("="*50)
    print(f"Model: {MODEL}")
