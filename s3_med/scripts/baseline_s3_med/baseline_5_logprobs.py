#!/usr/bin/env python3
"""
Baseline 5 with Token Probabilities: Similar Cases + LLM using logprobs (No RAG)
Uses similar patient cases with token probabilities for confidence
"""
...
def create_simple_prompt(patient_text, similar_cases):
    """Create simple prompt with similar cases for single-token response"""
    # Limit patient text
    ...
    prompt = f"""You are an expert ICU physician. Based on similar patient cases, predict ICU mortality risk.

Instructions:
- Consider the outcomes of similar patients with comparable clinical presentations
- Remember that LOW RISK is more common (87% of ICU patients survive)
- Only predict HIGH if similar cases show poor outcomes and current patient has severe indicators
...
def create_cot_prompt(patient_text, similar_cases):
    """Create COT prompt integrating similar cases"""
    # Limit patient text for COT to leave room for analysis
    if len(patient_text) > 1800:
        if "=== CLINICAL DATA ===" in patient_text:
            parts = patient_text.split("=== CLINICAL DATA ===")
            patient_text = parts[0] + "=== CLINICAL DATA ===" + parts[1][:1200]
        else:
            patient_text = patient_text[:1800]
    
    prompt = f"""You are an expert ICU physician. Analyze this patient using similar historical cases to predict ICU mortality step by step.
...
5. Similar case analysis:
   - Review outcomes of similar patients
   - Compare severity indicators with similar cases
   - Assess if current patient's condition aligns with high-risk or low-risk similar cases

Important:
- Remember that most ICU patients (87%) survive
- Base assessment on similar patient outcomes and clinical evidence
- Only predict HIGH if multiple severe indicators AND similar cases show poor outcomes
...
def parse_prediction_with_confidence(response_text, confidence):
    ...
    if "HIGH" in response_upper:
        ...
        return 0, confidence  # LOW RISK = survival
...
def main():
    parser = argparse.ArgumentParser(description="Baseline 5 with Logprobs: Similar Cases + LLM with Token Probabilities (No RAG)")
    ...
    parser.add_argument("--output_dir", default="/scratch/bcew/ruikez2/intern/s3_med/results/baseline_4_similar_cases_logprobs", 
    ...
    print(f"Processing {len(patients_text)} patients with similar cases and logprobs...")
    ...
    for i, patient in enumerate(tqdm(patients_text, desc="Processing patients")):
        ...
        if args.prompt_type == "cot":
            prompt = create_cot_prompt(patient_text, similar_cases_text)
        else:
            prompt = create_simple_prompt(patient_text, similar_cases_text)
        ...
        if len(predictions) == 1:
            ...
            print(f"Similar outcomes: {[sp['mortality'] for sp in similar_patients]}")
    ...
    output_file = os.path.join(args.output_dir, f'baseline_4_similar_cases_logprobs_{args.prompt_type}_results.json')
    ...
    # Print summary
    print("\n" + "="*50)
    print("BASELINE 5: SIMILAR CASES + LOGPROBS (NO RAG)")
    print("="*50)
    print(f"Model: {MODEL}")
