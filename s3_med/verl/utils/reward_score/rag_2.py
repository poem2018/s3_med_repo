...
##todot
def extract_mortality_prediction(solution_str):
    """
    TODO: Extract mortality prediction from Searcher output
    Looking for patterns like:
    - "Yes" / "No" 
    - "1" / "0"
    - "will die" / "will not die"
    - "high mortality risk" / "low mortality risk"
    
    Returns:
        1 for predicted death, 0 for predicted survival, -1 if no clear prediction
    """
    # Try to get the final response part after search completions
    try:
        if "Now, start the loop with the following question" in solution_str:
            response_str = solution_str.split("Now, start the loop with the following question")[1]
        else:
            response_str = solution_str
            
        # Look for the final answer after search_complete
        search_complete_match = re.search(r'<search_complete>.*?</search_complete>(.*)', response_str, re.DOTALL)
        if search_complete_match:
            final_response = search_complete_match.group(1).strip()
        else:
            # If no search_complete, use the last part of the response
            final_response = response_str.split('\n')[-10:]  # Last 10 lines
            final_response = '\n'.join(final_response)
    except:
        final_response = solution_str
    
    # Convert to lowercase for matching
    response_lower = final_response.lower()
    
    # Clear positive indicators (will die)
    positive_patterns = [
        r'\byes\b',
        r'\b1\b',
        r'will die',
        r'likely to die', 
        r'high mortality risk',
        r'high risk of mortality',
        r'poor prognosis',
        r'unlikely to survive',
        r'fatal outcome',
        r'expected to die'
    ]
    
    # Clear negative indicators (will survive)
    negative_patterns = [
        r'\bno\b',
        r'\b0\b',
        r'will not die',
        r'unlikely to die',
        r'low mortality risk',
        r'low risk of mortality',
        r'good prognosis',
        r'likely to survive',
        r'expected to survive',
        r'will survive'
    ]
    ...
    # If no clear prediction found
    print(f"[Warning] No clear mortality prediction found in response: {final_response[:200]}...")
    return -1
...
def compute_score_rag(solution_str, ground_truth, zeroshot_answers, data_source, use_utility_score=True, use_generation_score=True):
    ...
    """
    #todot
    if data_source == 'mimic_icu_mortality':
        # Extract prediction from Searcher output
        prediction = extract_mortality_prediction(solution_str)
        
        # Get ground truth mortality label
        true_mortality = ground_truth.get('mortality', ground_truth.get('mortality_inunit', 0))
        patient_id = ground_truth.get('patient_id', 'unknown')
        
        # Compute binary classification reward
        if prediction == -1:
            # Invalid/unclear prediction - negative reward
            score = -0.5
            is_correct = False
            prediction_text = "INVALID"
        elif prediction == true_mortality:
            # Correct prediction
            score = 1.0
            is_correct = True
            prediction_text = "DEATH" if prediction == 1 else "SURVIVAL"
        else:
        ...
        if do_print:
            print(f"--------------------------------")
            print(f"[Mortality Task] Patient ID: {patient_id}")
            print(f"Predicted: {prediction_text} ({prediction})")
            print(f"Ground Truth: {'DEATH' if true_mortality == 1 else 'SURVIVAL'} ({true_mortality})")
            print(f"Correct: {is_correct}")
            print(f"Score: {score}")
            if prediction == -1:
                print(f"[Warning] Could not extract clear prediction from output")
        
        return score, None, None
    
    # For non-mortality tasks, use original logic (can be restored later if needed)
    else:
        # Fallback to simple scoring for now
        print(f"[Warning] Non-mortality task {data_source} not fully supported yet")
    ...
    # TODO: New binary classification code for mortality prediction
    # Check if this is a mortality prediction task
    
    
    # # Original debug printing code
    # do_print = random.randint(1, 16) == 1
    #    
    # if use_utility_score:
    #     if do_print:
...
def output_sequence(solution_str, ground_truth):
    ...
    question = ground_truth['patient_id']  #todot
    golden_answers = ground_truth['answers'].tolist()
    ...
    if do_print:
        print(f"--------------------------------")
        # print(f"Question: {question}")
        print(f"Golden answers: {ground_truth['answers']}")
        print(f"Extracted doc_info: {context_with_info}")
