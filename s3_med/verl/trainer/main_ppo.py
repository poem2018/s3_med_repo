...
def _select_rm_score_fn(data_source):
    ...
    # TODOt: Use mortality-specific reward for MIMIC tasks
    if data_source == 'mimic_icu_mortality':
        # Use the modified compute_score_rag that handles binary classification
        return rag_2.compute_score_rag
    else:
        # For other tasks, still use the rag_2 function (can be modified later)
        return rag_2.compute_score_rag
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, format_score=0., zeroshot_cache_file="data/rag_cache/rag_cache.json", val_only=False, output_context_dir=None) -> None:
        ...
        ##todot: Track reward scores sum per epoch
        self.all_reward_scores = []
        self.epoch_score_sums = []  # Track sum of scores for each epoch
        self.current_epoch_scores = []  # Track scores within current epoch
    ...
    ##todot: Mark end of epoch and record epoch score sum
    def end_epoch(self):
        """Mark the end of an epoch and record the score sum"""
        if self.current_epoch_scores:
            epoch_sum = sum(self.current_epoch_scores)
            self.epoch_score_sums.append(epoch_sum)
            print(f"Epoch {self.current_epoch} completed: {len(self.current_epoch_scores)} samples, sum: {epoch_sum}")
            self.current_epoch_scores = []
            self.current_epoch += 1
    
    ##todot: Save reward scores to file
    def save_reward_scores(self):
        """Save all reward scores collected during training"""
        if not self.all_reward_scores:
            return
        
        # Save any remaining scores from current epoch
        if self.current_epoch_scores:
            self.end_epoch()
            
        scores_file = os.path.join(self.output_sequences_dir, "reward_scores.json")
        scores_data = {
            "reward_scores": self.all_reward_scores,
            "num_samples": len(self.all_reward_scores),
            "total_score": sum(self.all_reward_scores),
            "mean_score": sum(self.all_reward_scores) / len(self.all_reward_scores) if self.all_reward_scores else 0,
            "epoch_score_sums": self.epoch_score_sums,
            "num_epochs": len(self.epoch_score_sums)
        }
        
        with open(scores_file, 'w') as f:
            json.dump(scores_data, f, indent=2)
        print(f"Saved {len(self.all_reward_scores)} reward scores to {scores_file}")
        print(f"Epoch score sums: {self.epoch_score_sums}")
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if not self.val_only:
                ...
                ##todot: Track reward scores
                self.all_reward_scores.append(score)
                self.current_epoch_scores.append(score)
