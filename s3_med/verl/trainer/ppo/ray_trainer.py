...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            # TODO: generator_llm is optional for mortality binary classification
            generator_llm = getattr(self.config, 'generator_llm', None),  #todot
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ##todot: Notify reward function of epoch change
            if hasattr(self.reward_fn, 'end_epoch') and epoch > 0:
                self.reward_fn.end_epoch()
        ...
        ##todot: Save all retrieved documents and predictions after training completes
        if self.config.do_search and 'generation_manager' in locals() and hasattr(generation_manager, 'save_training_outputs'):
            output_dir = os.path.join(self.config.trainer.default_local_dir, 'training_outputs')
            print(f"##todot: Saving training outputs to {output_dir}")
            saved_files = generation_manager.save_training_outputs(output_dir=output_dir)
            print(f"##todot: Training outputs saved: {saved_files}")
        
        ##todot: Save reward scores after training completes
        if hasattr(self.reward_fn, 'save_reward_scores'):
            print(f"##todot: Saving reward scores...")
            self.reward_fn.save_reward_scores()
            print(f"##todot: Reward scores saved")
